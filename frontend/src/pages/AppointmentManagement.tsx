...
const AppointmentManagement: React.FC = () => {
  ...
  const fetchDoctors = async () => {
    try {
      // Load all active doctors so they are selectable even if temporarily unavailable
  ...
  const handleCreateAppointment = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const patientId = parseInt(newAppointment.patient);
      const doctorId = parseInt(newAppointment.doctor);
      const fee = parseFloat(newAppointment.consultation_fee.toString());
